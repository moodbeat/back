...
class CompletedSurvey(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        опроса.
        """
        # временный костыль, сделаю по нормальному
        if self.survey.type.slug == 'yn':
            values = [item['variant_value'] for item in self.results]
            result_in_persent = (
                values.count(1) / self.survey.questions.count() * 100
            )
            self.set_mental_state(result_in_persent)
        ...
        if self.survey.type.slug == 'mbi':
            ...
            questions = [item['question_id'] for item in self.results]
